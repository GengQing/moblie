...
若$y_1(x),y_2(x)$均是齐次方程的解,则它们的任意线性组合也是齐次解,如$y_1(x)+y_2(x),2y_1(x)$等.
...
$n$阶行列式是所以取自不同行不同列$n$个元素乘积的代数和.  
逆序数为行标的逆序数与列标的逆序数之和,
行(列)逆序数为行(列)下标后面元素比当前元素小的元素的个数之和
...
4. 两同型矩阵所以元素都相等,则两矩阵<u>相等</u>

### 矩阵的运算的定义

1. 加法,是指两同型矩阵相加,等于把对应位置的元素相加
2. 数乘,是指数与矩阵的乘法,等于把矩阵中每个元素与数相乘
3. 矩阵乘法,矩阵$A$的行数等于矩阵$B$的列数才能相乘,$A$的行去乘以$B$的列
4. 矩阵转置是指把矩阵的行列互换
...
2 一个线性无关组的极大线性无关组是该向量组本身  
3 向量组的极大线性无关组一般不唯一,但极大线性无关组的向量个数是一样的.  
4 极大线性无关组中向量的个数即为向量组的秩.
...
1. 利用初等行变换不改变线性方程组的解,将$A$作初等变换化成阶梯形矩阵,得通解阶梯形方程组  
