...
一 证明两矩阵有相同的特征多项式,即$|\lambda E-A|=|\lambda E-B|$.  
二 根据相似的定义证明两矩阵相似,如$A$是可逆矩阵,则$AB\sim BA$  
三 零矩阵的全部特征值都是零.

### 判断矩阵$A$是否相似于对角矩阵的步骤

(1) 看$A$是否是实对称矩阵,实对称矩阵必相似于对角矩阵

(2) $A$不是实对称矩阵时,看$A$是否有$n$个互不相同的特征值,  
...
1 转化成齐次方程组,证明方程组有相同的解,进而有相同的基础解系,则秩相等.  

2 如果极大线性无关组个数相等则秩相等,反之不相等

3 可对角化,如果非零特征值个数相等则秩相等,反之不相等
