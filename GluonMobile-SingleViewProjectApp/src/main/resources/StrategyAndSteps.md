...
2. 画出积分区域草图,判断积分区域是否具有对称性,是否(或平移换元后)可拆分成X轴对称,Y轴对称以及关于y=x对称
3. 被积函数是否具有奇偶性,利用被积函数的奇偶性和积分区间的对称性计算
...
1. 向$x$轴正方向平移$n$个单位得:$f(x-n,y)$
2. 向$y$轴正方向平移$n$个单位得:$f(x,y-n)$
3. 向$x$轴负方向平移$n$个单位得:$f(x+n,y)$
4. 向$y$轴负方向平移$n$个单位得:$f(x,y+n)$
