...
3. 可去间断点函数,在间断点$x_0$的领域有定义,在其空心领域内可导,导函数在该点的极限存在,
但在该点不可导,不连续,如$f(x)=\frac {x^2}{x}且f(0)=1$
4. 振荡型可去间断点函数如下, 补充定义后在间断点的领域有定义,在该点可导,连续,
但在其导函数在该点极限不存在,在该点不连续
...
2. 令$y'=0$带入方程$II,得到方程III$
3. 联立方程$I和III解出x和y$, 得到若干驻点
4. 验证$y'$的分母不能为0
...
3. 比较1和2中各个值得大小,取最大值和最小值.
4. 如果$f(x)$在区间内部只有一个可疑极值点，并且是极大(极大小)值,则它必是$f(x)$的最大(最小)值点,区间可以是开区间，闭区间，半开半闭区间以及无穷区间.实际上遇到的多数是4.
...
1. 最小值$\ge 0, 则f(x)\ge 0$
2. 最大值$\le 0, 则f(x)\le 0$
