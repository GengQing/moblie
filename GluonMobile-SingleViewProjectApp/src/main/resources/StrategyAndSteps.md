...
3. 根据二重积分的几何意义,若需平移,将被积函数平移,如平移后$f(x-1,y), X=x-1,$看被积函数$f(X,y)$是否具有奇偶性,利用被积函数的奇偶性和积分区间的对称性计算
...
1. 根据参数方程确定$x$的范围
2. 将参数方程看成$y=y(x)$
3. 写成累次积分,先积分$y$,再积分$x, \int_a^b dx\int_d^{y(x)}f(x,y)dy$.
4. 积分$x$使用换元积分带入参数方程$\int_{t_1}^{t_2} g[x(t),y(t))]x'(t)dt$
