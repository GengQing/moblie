...
第一步 认真阅题,提取条件,将参数条件用笔画出.  
看参数是$\gt 0 还是 lt 0$, 是对称,还是非对称.不可轻敌;  
第二步 看是选择正确的还是选择错误的;  
第三步 选择解题方法,解题方法有: 
...
2. 设$f(x),在x=x_0$某领域内有定义, 在其去心领域内可导.  
    ...
    (2)若$\lim_{x\rightarrow x_0} f'(x)=A \nRightarrow f'(x_0)$存在且等于A.反例:符号函数.  
...
$$
I=\int_0^b f(x)dx  {x=b-t \over =}\int_0^b f(b-t)dt=\int_0^b f(b-x)dx\\
...
1 幂等矩阵的特征值范围是$0$或$1,A=0或(E-A)=0$.

2 幂等矩阵可相似对角化

3 幂等矩阵的迹等于幂等矩阵的秩,即$tr(A)=rank(A)$

4 可逆的幂等矩阵只有$E$,反之若幂等矩阵不等于$E$,则不可逆.
...
2 若行列式的符号(正负)不同,则不合同.

3 实对称矩阵合同的充分必要条件是它们的正负惯性指数相同  
(可用配方法求惯性指数)
...
2 对多项式形式$f$,若能找到非零向量$X,使f(x_1,\cdots) \le 0$,则不是正定的.

3 若主对角元素$a_{ii} \le 0$,则不是正定的.

4 检查顺序主子式是否都大于0,如果不是则不是正定的.

5 $A=D^\intercal D,D$是可逆矩阵,则正定.

6 正惯性指数$=n$,或特征根都$\lambda>0$,或与$E$合同,是正定的.
